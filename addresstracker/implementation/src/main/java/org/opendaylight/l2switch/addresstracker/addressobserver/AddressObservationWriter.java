...
/**
 * AddressObservationWriter manages the MD-SAL data tree for address observations (mac, ip) on each
 * node-connector.
 */
public class AddressObservationWriter {
  ...
  private final Map<NodeConnectorLock, FluentFuture<CommitInfo>> futureMap =
   ...
   */
  public void addAddress(MacAddress macAddress, IpAddress ipAddress,
    ...
    NodeConnectorLock nodeConnectorLock =
        lockMap.computeIfAbsent(nodeConnectorRef, key -> new NodeConnectorLock());
    ...
    synchronized (nodeConnectorLock) {
      ...
      final AddressCapableNodeConnectorBuilder acncBuilder =
          new AddressCapableNodeConnectorBuilder();
      final AddressesBuilder addressBuilder = new AddressesBuilder().setIp(ipAddress)
          .setMac(macAddress).setFirstSeen(now).setLastSeen(now);
      ...
      try {
        ...
        throw new RuntimeException(
            "Error reading from operational store, node connector : " + nodeConnectorRef, e);
      }
      ...
      DataObjectIdentifier<AddressCapableNodeConnector> addressCapableNcInstanceId =
          ((DataObjectIdentifier<AddressCapableNodeConnector>) nodeConnectorRef.getValue());
      ...
      writeTransaction.merge(LogicalDatastoreType.OPERATIONAL, addressCapableNcInstanceId,
          acncBuilder.build());
