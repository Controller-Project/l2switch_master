...
/**
 * ProactiveFloodFlowWriter is used for the proactive mode of L2Switch. In this mode, flood flows
 * are automatically written to each switch and less traffic is sent to the controller.
 */
public class ProactiveFloodFlowWriter
    implements DataTreeChangeListener<StpStatusAwareNodeConnector>,
  ...
  private final ScheduledExecutorService stpStatusDataChangeEventProcessor =
  ...
  private final class StpStatusDataChangeEventProcessor implements Runnable {
    ...
    public void run() {
      ...
      if (threadReschedule) {
        ...
        stpStatusDataChangeEventProcessor.schedule(this, flowInstallationDelay,
        ...
        for (Node node : nodes.nonnullNode().values()) {
          ...
          if (nodeConnectors != null) {
            for (NodeConnector outerNodeConnector : nodeConnectors.values()) {
              StpStatusAwareNodeConnector outerSaNodeConnector =
                  outerNodeConnector.augmentation(StpStatusAwareNodeConnector.class);
              ...
              if (!outerNodeConnector.getId().toString().contains("LOCAL")) {
                ...
                for (NodeConnector nodeConnector : nodeConnectors.values()) {
                  if (!nodeConnector.getId().toString().contains("LOCAL")
                    ...
                    // on
                    StpStatusAwareNodeConnector saNodeConnector =
                        nodeConnector.augmentation(StpStatusAwareNodeConnector.class);
                    if (saNodeConnector == null
                        || StpStatus.Forwarding.equals(saNodeConnector.getStatus())) {
                      outputActions.add(new ActionBuilder().setOrder(order++)
                          .setAction(new OutputActionCaseBuilder()
                              .setOutputAction(
                                  new OutputActionBuilder().setMaxLength(Uint16.MAX_VALUE)
                                      .setOutputNodeConnector(nodeConnector.getId()).build())
                              .build())
                          .build());
                    }
                  }
                ...
                if (outerSaNodeConnector == null) {
                  outputActions
                      .add(new ActionBuilder().withKey(new ActionKey(order++))
                          .setAction(new OutputActionCaseBuilder()
                              .setOutputAction(
                                  new OutputActionBuilder().setMaxLength(Uint16.MAX_VALUE)
                ...
                ApplyActions applyActions =
                    new ApplyActionsBuilder().setAction(BindingMap.ordered(outputActions)).build();
    ...
    /**
     * Starts and commits data change transaction which modifies provided flow path with supplied
     * body.
     */
    private Future<RpcResult<AddFlowOutput>> writeFlowToSwitch(NodeId nodeId, Flow flow) {
      DataObjectIdentifier<Node> nodeInstanceId =
          DataObjectIdentifier.builder(Nodes.class).child(Node.class, new NodeKey(nodeId)).build();
      DataObjectIdentifier<Table> tableInstanceId =
          nodeInstanceId.toBuilder().augmentation(FlowCapableNode.class)
      ...
      final AddFlowInputBuilder builder = new AddFlowInputBuilder(flow)
          .setNode(new NodeRef(nodeInstanceId)).setFlowTable(new FlowTableRef(tableInstanceId))
          .setFlowRef(new FlowRef(flowPath)).setTransactionUri(new Uri(flow.getId().getValue()));
