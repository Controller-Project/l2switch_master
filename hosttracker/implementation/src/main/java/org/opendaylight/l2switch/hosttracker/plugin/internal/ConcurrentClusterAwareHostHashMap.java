...
/**
 * This will (try to) submit all writes and deletes in to the MD-SAL database. The removeLocally and
 * putLocally methods should be used when dataChanges are dealt locally and not update to MD-SAL.
 */
public class ConcurrentClusterAwareHostHashMap {
  private static final Logger LOG =
      LoggerFactory.getLogger(ConcurrentClusterAwareHostHashMap.class);
   ...
   */
  private final ConcurrentHashMap<DataObjectIdentifier<Node>, HostId> instanceIDs =
  ...
  /**
   * Removes, if exists, the Host with the given InstanceIdentifier&lt;Node&gt; from this local
  ...
  /**
   * Removes, if exists, the Host with the given Key (HostId) from this local HashMap. Ideally used
   * for host data listener events.
   *
   * @param key the key (HostId) of the Host to remove.
   * @return the removed Host if exits, null if it doesn't exist.
   */
  public synchronized Host removeLocally(HostId key) {
    Iterator<Entry<DataObjectIdentifier<Node>, HostId>> iterator =
        this.instanceIDs.entrySet().iterator();
  ...
  /**
   * Puts the given value (Host) only in this local HashMap. Ideally used for host data listener
   * events.
   *
      ...
      for (Host h : hosts) {
        if (e.getValue().equals(h.getId())) {
          this.opProcessor.enqueueOperation(
              tx -> tx.addDeleteOperationToTxChain(LogicalDatastoreType.OPERATIONAL, e.getKey()));
  ...
  /**
   * Forces the local Host with the given HostId to be merged into MD-SAL database.
   *
  ...
  public synchronized void submit(HostId hostid) {
    ...
    final DataObjectIdentifier<Node> buildNodeIID =
        Utilities.buildNodeIID(hostNode.key(), topologyId);
    this.opProcessor.enqueueOperation(tx -> tx.mergeToTransaction(LogicalDatastoreType.OPERATIONAL,
        buildNodeIID, hostNode, true));
  ...
  /**
   * Puts all the given hosts into this local HashMap and into MD-SAL database.
   *
  ...
  public synchronized void putAll(List<Host> hosts) {
    for (Host h : hosts) {
      ...
      final DataObjectIdentifier<Node> buildNodeIID =
          Utilities.buildNodeIID(hostNode.key(), topologyId);
      this.opProcessor.enqueueOperation(tx -> tx
          .mergeToTransaction(LogicalDatastoreType.OPERATIONAL, buildNodeIID, hostNode, true));
  ...
  /**
   * Removes the value (Host) with the given hostId from this local HashMap and MD-SAL database.
   *
  ...
  public synchronized Host remove(HostId hostId) {
    ...
    if (removedValue != null) {
      ...
      this.opProcessor.enqueueOperation(
          tx -> tx.addDeleteOperationToTxChain(LogicalDatastoreType.OPERATIONAL, hnIID));
  ...
  /**
   * Removes all of the mappings from this local HashMap and from MD-SAL. The local HashMap will be
   * empty after this call returns.
   */
  public synchronized void clear() {
    for (final Map.Entry<? extends DataObjectIdentifier<Node>, HostId> e : this.instanceIDs
        .entrySet()) {
      this.opProcessor.enqueueOperation(
          tx -> tx.addDeleteOperationToTxChain(LogicalDatastoreType.OPERATIONAL, e.getKey()));
    }
