...
public class Ipv6Decoder extends AbstractPacketDecoder<EthernetPacketReceived, Ipv6PacketReceived>
  ...
  public Ipv6PacketReceived decode(EthernetPacketReceived ethernetPacketReceived) {
    ...
    try {
      ...
      builder.setDscp(
          new Dscp(BitBufferHelper.getUint8(BitBufferHelper.getBits(data, bitOffset + 4, 6))));
      builder.setEcn(BitBufferHelper.getUint8(BitBufferHelper.getBits(data, bitOffset + 10, 2)));
      builder.setFlowLabel(
          BitBufferHelper.getUint32(BitBufferHelper.getBits(data, bitOffset + 12, 20)));
      builder.setIpv6Length(
      ...
      builder
          .setHopLimit(BitBufferHelper.getUint8(BitBufferHelper.getBits(data, bitOffset + 56, 8)));
      builder.setSourceIpv6(Ipv6Address.getDefaultInstance(InetAddress
          .getByAddress(BitBufferHelper.getBits(data, bitOffset + 64, 128)).getHostAddress()));
      builder.setDestinationIpv6(Ipv6Address.getDefaultInstance(InetAddress
          .getByAddress(BitBufferHelper.getBits(data, bitOffset + 192, 128)).getHostAddress()));
      ...
      while (nextHeader != null && !nextHeader.equals(KnownIpProtocols.Tcp)
        ...
        extensionHeaders.add(new ExtensionHeadersBuilder().setNextHeader(nextHeader)
            .setLength(Uint16.valueOf(octetLength)).setData(Arrays.copyOfRange(data, start, end))
            .build());
