...
/**
 * A base class for all decoders. Each extended decoder should also implement a notification
 * listener that it can consume.
 */
...
public abstract class AbstractPacketDecoder<C, P extends Notification> implements AutoCloseable {
  ...
  public AbstractPacketDecoder(Class<P> producedPacketNotificationType,
    ...
    listenerRegistration =
        this.notificationService.registerListener(getPacketType(), getConsumedListener());
  }
  ...
  // listenerRegistration =
  // notificationProviderService.registerNotificationListener(notificationListener);
  // }
  // }
  // }
   ...
   * This method would make sure it decodes only when necessary and publishes corresponding event on
   * successful decoding.
   */
  ...
  /**
   * Decodes the payload in given Packet further and returns a extension of Packet. e.g. ARP, IPV4,
   * LLDP etc.
   */
