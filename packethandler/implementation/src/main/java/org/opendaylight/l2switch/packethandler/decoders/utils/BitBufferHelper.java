...
/**
 * BitBufferHelper class that provides utility methods to - fetch specific bits from a serialized
 * stream of bits - convert bits to primitive data type - like short, int, long - store bits in
 * specified location in stream of bits - convert primitive data types to stream of bits.
 */
public final class BitBufferHelper {
  ...
  /**
   * Returns the Uint8 value for the byte array passed. Size of byte array is restricted to 8 bits
   * (1 byte).
   *
   * @param data the byte array
   * @return Uint8 value
  ...
  /**
   * Returns the Uint16 value for the byte array passed. Size of byte array is restricted to 16 bits
   * (2 bytes).
   *
   * @param data the byte array
   * @return Uint16 value
  ...
  /**
   * Returns the Uint16 value for the last numBits of the byte array passed. Size of numBits is
  ...
  /**
   * Returns the Uint32 value for the byte array passed. Size of byte array is restricted to 32 bits
   * (4 bytes).
   *
   * @param data the byte array
   * @return Uint32 value
  ...
  public static long getLong(byte[] data, int numBits) {
    ...
    if (numBits > data.length * NetUtils.NUM_BITS_IN_A_BYTE) {
      LOG.error("getLong",
          new BufferException("Trying to read more bits than contained in the data buffer"));
    }
    int startOffset = data.length * NetUtils.NUM_BITS_IN_A_BYTE - numBits;
    byte[] bits = null;
    try {
  ...
  public static byte[] getBits(byte[] data, int startOffset, int numBits) throws BufferException {
    ...
    int numBytes =
        numBits % NetUtils.NUM_BITS_IN_A_BYTE != 0 ? 1 + numBits / NetUtils.NUM_BITS_IN_A_BYTE
    ...
    if (extraOffsetBits == 0) {
      if (extranumBits == 0) {
        ...
        bytes[numBytes - 1] =
            (byte) (data[startByteOffset + numBytes - 1] & getMSBMask(extranumBits));
      }
    } else {
      int index;
      int valfromnext;
      ...
      if (extranumBits != 0) {
        if (extranumBits < NetUtils.NUM_BITS_IN_A_BYTE - extraOffsetBits) {
          valfromnext =
              (byte) (data[startByteOffset + index] & getMSBMask(extranumBits) >> extraOffsetBits);
    ...
    if (numBits % NetUtils.NUM_BITS_IN_A_BYTE == 0) {
      ...
      numBitstoShiftBy =
          NetUtils.NUM_BITS_IN_A_BYTE - numBits % NetUtils.NUM_BITS_IN_A_BYTE < leadZeroesMSB
              ? NetUtils.NUM_BITS_IN_A_BYTE - numBits % NetUtils.NUM_BITS_IN_A_BYTE
              : leadZeroesMSB;
    }
      ...
      for (index = 0; index < size - 1; index++) {
        if (index + 1 == size - 1) {
          if (numEndRestBits > numBitstoShiftBy) {
            shiftedBytes[index] = (byte) (inputBytes[index] << numBitstoShiftBy
                | (inputBytes[index + 1] & getMSBMask(numBitstoShiftBy)) >> numEndRestBits
                    - numBitstoShiftBy);
            shiftedBytes[index + 1] = (byte) ((inputBytes[index + 1]
                & getLSBMask(numEndRestBits - numBitstoShiftBy)) << numBitstoShiftBy);
          } else {
            shiftedBytes[index] =
                (byte) (inputBytes[index] << numBitstoShiftBy | (inputBytes[index + 1]
                    & getMSBMask(numEndRestBits)) >> NetUtils.NUM_BITS_IN_A_BYTE - numEndRestBits);
          }
        }
        shiftedBytes[index] = (byte) (inputBytes[index] << numBitstoShiftBy
            | (inputBytes[index + 1] & getMSBMask(numBitstoShiftBy)) >> NetUtils.NUM_BITS_IN_A_BYTE
                - numBitstoShiftBy);
      }
  ...
  /**
   * It aligns the first numBits bits to the right end of the byte array preceding them with numBits
   * % 8 zero bits.
   *
   * <p>
  ...
  /**
   * Insert in the data buffer at position dictated by the offset the number of bits specified from
   * the input data byte array. The input byte array has the bits stored starting from the LSB.
   *
   * @param data the byte array
  ...
  public static void insertBits(byte[] data, byte[] inputdataLSB, int startOffset, int numBits) {
    ...
    if (extraOffsetBits == 0) {
      ...
      for (index = 0; index < numBytes; index++) {
        ...
        inputMSBbits =
            (byte) (inputdata[index] & getMSBMask(NetUtils.NUM_BITS_IN_A_BYTE - extraOffsetBits));
      ...
      if (restBits < NetUtils.NUM_BITS_IN_A_BYTE - extraOffsetBits) {
        ...
        inputLSBbits = inputdata[index] & getLSBMask(restBits
            - (NetUtils.NUM_BITS_IN_A_BYTE - extraOffsetBits)) << NetUtils.NUM_BITS_IN_A_BYTE
                - restBits;
  ...
  public static void checkExceptions(byte[] data, int startOffset, int numBits)
    ...
    int endOffsetByte;
    int startByteOffset;
    endOffsetByte = startOffset / NetUtils.NUM_BITS_IN_A_BYTE
        + numBits / NetUtils.NUM_BITS_IN_A_BYTE + (numBits % NetUtils.NUM_BITS_IN_A_BYTE != 0 ? 1
    ...
    if (startOffset < 0 || startByteOffset >= data.length || endOffsetByte > data.length
        || numBits < 0 || numBits > NetUtils.NUM_BITS_IN_A_BYTE * data.length) {
      throw new BufferException("Illegal arguement/out of bound exception - data.length = "
          + data.length + " startOffset = " + startOffset + " numBits " + numBits);
    }
  }
}
