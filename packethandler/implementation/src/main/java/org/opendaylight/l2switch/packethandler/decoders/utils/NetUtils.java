...
/**
 * Utility class containing the common utility functions needed for operating on networking data
 * structures.
 */
public final class NetUtils {
   ...
   * @return The long number. Zero is returned if {@code ba} is {@code null} or the length of it is
   *         not six.
   */
  ...
  /**
   * Return the InetAddress Network Mask given the length of the prefix bit mask. The prefix bit
   * mask indicates the contiguous leading bits that are NOT masked out. Example: A prefix bit mask
   * length of 8 will give an InetAddress Network Mask of 255.0.0.0
   *
  ...
  /**
   * Returns the prefix size in bits of the specified subnet mask. Example: For the subnet mask
   * ff.ff.ff.e0 it returns 25 while for ff.00.00.00 it returns 8. If the passed subnetMask array is
   * null, 0 is returned.
   *
  ...
  /**
   * Returns the prefix size in bits of the specified subnet mask. Example: For the subnet mask
   * 255.255.255.128 it returns 25 while for 255.0.0.0 it returns 8. If the passed subnetMask object
  ...
  /**
   * Given an IP address and a prefix network mask length, it returns the equivalent subnet prefix
   * IP address Example: for ip = "172.28.30.254" and maskLen = 25 it will return "172.28.30.128".
   *
   ...
   *
   * <p>
   * For example: testAddress: 172.28.2.23 testMask: 255.255.255.0 filterAddress: 172.28.1.10
   * testMask: 255.255.255.0 do conflict
   *
   * <p>
   * testAddress: 172.28.2.23 testMask: 255.255.255.0 filterAddress: 172.28.1.10 testMask:
   * 255.255.0.0 do not conflict
   *
   * <p>
  ...
  public static boolean inetAddressConflict(InetAddress testAddress, InetAddress filterAddress,
      InetAddress testMask, InetAddress filterMask) {
    // Sanity check
  ...
  /**
   * Returns true if the MAC address is a multicast MAC address and false otherwise. Note that this
   * explicitly returns false for the broadcast MAC address.
   *
   * @param macAddress the MAC address
   */
  ...
  /**
   * Checks if the passed IP v4 address in string form is valid The address may specify a mask at
   * the end as "/MM".
   *
  ...
  /**
   * Checks if the passed IP v6 address in string form is valid The address may specify a mask at
   * the end as "/MMM".
   *
  ...
  /**
   * Checks if the passed IP address in string form is a valid v4 or v6 address. The address may
   * specify a mask at the end as "/MMM".
   *
  ...
  /*
   * Following utilities are useful when you need to compare or bit shift java primitive type
   * variable which are inherently signed
   */
